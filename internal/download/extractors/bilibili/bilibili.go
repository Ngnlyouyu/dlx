...
func extractBangumi(url, html string, extractOption datafile.Options) ([]*datafile.Data, error) {
	...
	if !extractOption.Playlist {
		...
		return []*datafile.Data{bilibiliDownload(options, extractOption)}, nil
	}
	...
	extractedData := make([]*datafile.Data, len(needDownloadItems))
	wg := sync.WaitGroup{}
	dataIndex := 0
	for index, u := range data.EpList {
		...
		go func(index int, options bilibiliOptions, extractedData []*datafile.Data) {
			defer wg.Done()
...
func extractNormalVideo(url, html string, extractOption datafile.Options) ([]*datafile.Data, error) {
	...
	if !extractOption.Playlist {
		...
		if len(pageData.VideoData.Pages) < p || p < 1 {
			return nil, datafile.ErrURLParseFailed
		}
...
func multiEpisodeDownload(url, html string, extractOption datafile.Options, pageData *multiPage) ([]*datafile.Data, error) {
...
func multiPageDownload(url, html string, extractOption datafile.Options, pageData *multiPage) ([]*datafile.Data, error) {
...
func New() datafile.Extractor {
	return &extractor{}
}

// Extract is the main function to extract the data.
func (e *extractor) Extract(url string, option datafile.Options) ([]*datafile.Data, error) {
	var err error
...
func bilibiliDownload(options bilibiliOptions, extractOption datafile.Options) *datafile.Data {
	var (
		err  error
		html string
	)
	if options.html != "" {
		...
		if err != nil {
			return datafile.EmptyData(options.url, err)
		}
	}
	...
	streams := make(map[string]*datafile.Stream, len(dashData.Quality))
	for _, stream := range dashData.Streams.Video {
		s, err := request.Size(stream.BaseURL, referer)
		if err != nil {
			return datafile.EmptyData(options.url, err)
		}
		parts := make([]*datafile.Part, 0, 2)
		parts = append(parts, &datafile.Part{
			URL:  stream.BaseURL,
			Size: s,
	...
	for _, durl := range dashData.DURLs {
		...
		streams[strconv.Itoa(dashData.CurQuality)] = &datafile.Stream{
			Parts:   parts,
			Size:    durl.Size,
	...
	return &datafile.Data{
		...
		Title:   title,
		Type:    datafile.DataTypeVideo,
		Streams: streams,
		Captions: map[string]*datafile.CaptionPart{
			"danmaku": {
				Part: datafile.Part{
...
func getSubTitleCaptionPart(aid int, cid int) *datafile.CaptionPart {
